...
namespace io
{
    ...
    {
        int32_t out;
        __asm__ volatile ("call 1f \n\t"	//cant be accessed directly, call smth (eip gets pushed), and then just pop
            "1: pop %0" : "=r"(out));
        return out;
    };
    ...
    {
        asm_outb(CMOS_ADDRESS, (1 << 7) | (offset & B(01111111)));     //select register
        /*uint32_t i = 0;
        while (i++ < 50000);                //CMOS is not the fastest TODO: see below*/

        return asm_inb(CMOS_DATA);
    };
